...
public final class Vectors {
     ...
     * defined by {@link Scalars#EPSILON}. Although geometrically accurate in Euclidean space,
     ...
     * is less than {@link Scalars#EPSILON} raised to the power of {@code 2}
    ...
    public static <V extends Vector<V>> boolean looseEquals(@Nonnull V v1, @Nonnull V v2) {
        return v1.distance2(v2) < (Scalars.EPSILON * Scalars.EPSILON);
    }

    /**
     * Checks for loose equality between the provided values {@code v1} and {@code v2}.
     * This account for potential floating point imprecision by accepting a small margin of error
     * defined by {@link Scalars#EPSILON}. Unlike {@link #looseEquals(Vector, Vector)}, this uses the
     * Manhattan distance between the two vectors, and may lead to unexpected behavior.
     *
     * @param v1  The first value to compare
     * @param v2  The second value to compare
     * @param <V> The type of vector to compare
     * @return {@code true} if the Manhattan distance between the two vectors is less than {@link Scalars#EPSILON}
     * @throws IllegalArgumentException When the vector is a variable size vector, and the dimensions
     *                                  (the number of scalar components) of the two vectors are different
     * @see #looseEquals(Vector, Vector)
     */
    public static <V extends Vector<V>> boolean fastEquals(@Nonnull V v1, @Nonnull V v2) {
        return v1.distanceManhattan(v2) < Scalars.EPSILON;
    }
    ...
    public static <V extends Vector<V>> Vector<?> sum(@Nonnull V... values) {
        ...
        final double[] components = values[0].array();
        ...
        for (V value : values) {
            ...
            for (int j = 1; j < value.dimensions(); j++) {
                components[j] = components[j] + value.get(j);
            }
        }
    ...
    public static <V extends Vector<V>> Vector<?> min(@Nonnull V... values) {
        ...
        for (final V value : values) {
            ...
            for (int i = 0; i < value.dimensions(); i++) {
                components[i] = Math.min(components[i], value.get(i));
            }
        }
    ...
    public static <V extends Vector<V>> Vector<?> max(@Nonnull V... values) {
        ...
        for (final V value : values) {
            ...
            for (int i = 0; i < value.dimensions(); i++) {
                components[i] = Math.max(components[i], value.get(i));
            }
        }
