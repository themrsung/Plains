...
public class DoubleArray implements SafeArray<Double> {
    ...
    @Override
    @SuppressWarnings("unchecked")
    public <F> Tuple<F> mapToTuple(@Nonnull Function<? super Double, ? extends F> f) {
    ...
    @Nonnull
    @Override
    public SafeArray<Double> append(@Nonnull SafeArray<? extends Double> a) {
        final int length = elements.length + a.length();
        final DoubleArray result = new DoubleArray(length);
    ...
    @Nonnull
    @Override
    public SafeArray<Double> prepend(@Nonnull SafeArray<? extends Double> a) {
        final int length = elements.length + a.length();
        final DoubleArray result = new DoubleArray(length);
     ...
     */
    @Override
    public void forEach(@Nonnull BiConsumer<Integer, ? super Double> action) {
        for (int i = 0; i < elements.length; i++) {
