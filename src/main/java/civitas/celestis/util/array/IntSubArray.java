...
/**
 * A int array which directly references its parent array's internal array.
 ...
 * @see IntArray
 */
class IntSubArray implements IntArray {
    //
    // Constants
    //
     ...
     */
    IntSubArray(@Nonnull int[] original, int startingIndex, int endingIndex) {
    ...
    public void update(int i, @Nonnull IntUnaryOperator f) throws IndexOutOfBoundsException {
        final int adjusted = i + startingIndex;
        if (adjusted >= endingIndex) throw new ArrayIndexOutOfBoundsException(i);
        original[adjusted] = f.applyAsInt(original[adjusted]);
    }
    ...
    public void update(@Nonnull IntUnaryOperator f) {
        for (int i = startingIndex; i < endingIndex; i++) {
            original[i] = f.applyAsInt(original[i]);
        }
    }
    ...
    public IntArray subArray(int s, int e) throws IndexOutOfBoundsException {
        return new IntSubArray(original, s + startingIndex, e + startingIndex);
    }
    ...
    public IntArray resize(int size) {
        final IntFastArray result = new IntFastArray(size);
    ...
    @Override
    public void sort() {
        final int[] sorted = stream().sorted().toArray();
    ...
    public void sort(@Nonnull Comparator<? super Integer> c) {
        final int[] sorted = stream().boxed().sorted(c).mapToInt(Integer::intValue).toArray();
        System.arraycopy(sorted, 0, original, startingIndex, endingIndex - startingIndex);
    }
    ...
    public IntArray merge(@Nonnull IntArray a, @Nonnull IntBinaryOperator f)
        ...
        for (int i = startingIndex; i < endingIndex; i++) {
            result.values[i - startingIndex] = f.applyAsInt(original[i], a.get(i - startingIndex));
        }
    ...
    @Override
    public int[] array() {
        final int[] result = new int[endingIndex - startingIndex];
