...
/**
 * An array-typed static grid which uses the type {@code long}.
 *
 * @see Grid
 */
public class LongGrid implements Grid<Long> {
    //
    // Constants
    //
    ...
    /**
     * Creates a new long grid from a 2D array of values.
     *
     * @param values The values of which to contain in the grid
     * @return The constructed grid
     */
    @Nonnull
    public static LongGrid of(@Nonnull long[][] values) {
        final int rows = values.length;
        final int columns = rows > 0 ? values[0].length : 0;
        final LongGrid grid = new LongGrid(rows, columns);
     ...
     */
    @Override
    public Long getOrDefault(int r, int c, Long ignored) throws IndexOutOfBoundsException {
        return values[r][c];
    }
     ...
     */
    @Override
    public void apply(@Nonnull TriFunction<Integer, Integer, ? super Long, Long> f) {
        for (int r = 0; r < rows; r++) {
            for (int c = 0; c < columns; c++) {
    ...
    @Nonnull
    @Override
    public Grid<Long> subGrid(int r1, int c1, int r2, int c2) throws IndexOutOfBoundsException {
     ...
     */
    @Override
    public void setRange(int r1, int c1, int r2, int c2, @Nonnull Grid<? extends Long> g)
    ...
    @Nonnull
    @Override
    public Grid<Long> resize(int rows, int columns) {
    ...
    @Nonnull
    @Override
    public <F, G> Grid<G> merge(@Nonnull Grid<F> g, @Nonnull BiFunction<? super Long, ? super F, G> f)
    ...
    @Nonnull
    @Override
    public SafeArray<Long> safeArray() {
